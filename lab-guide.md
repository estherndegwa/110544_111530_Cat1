...
- Optional: Node.js (v18+) if you want to run the sample API

## What you’ll do
1. Set up MongoDB in Docker and seed sample data
2. Explore the Document Data Model basics
3. Perform CRUD operations using Mongo Shell and API routes
...
- `api/` — optional Node.js Express API to demonstrate CRUD via HTTP
...
5) Optional: Start the sample API
```bash
docker compose -f api/docker-compose.api.yml up -d --build
```
...
- HTTP API endpoints (optional Node.js Express service)
...
### 3.2 CRUD via HTTP (Optional API)
If you started the API, try:
...
curl -X POST http://localhost:3000/products \
  ...
  -d '{
    ...
    "tags":["ergonomic","rgb"],
    "ratings":{"average":4.2,"count":210}
  }'
```

Read:
```bash
curl http://localhost:3000/products/SKU-3001
```

Update:
```bash
curl -X PATCH http://localhost:3000/products/SKU-3001 \
  -H "Content-Type: application/json" \
  -d '{"price":54.99,"in_stock":false}'
```

Delete:
```bash
curl -X DELETE http://localhost:3000/products/SKU-3001
```
...
- Mongo Express dashboard showing `shop` collections
- Product document view (`SKU-2001`)
- Aggregation result showing joined reviews
- API responses from curl (terminal output)

Place your images in `screenshots/` and reference them below:
- ![image1](screenshots/mongo-express-aggregation.png)
- ![image2](screenshots/mongo-express-products.png)
- ![image3](screenshots/api-get-product.png)
- ![image4](screenshots/docker-ps.png)

Note: Image numbering is reverse of upload order in this document.
...
- Optional API for HTTP-based CRUD

If anything fails:
- Run `docker compose logs mongo` and `docker compose logs mongo-express`
- Ensure ports 27017, 8081, and 3000 (optional) are free
